#!/bin/bash
# Minimal E2SAR sender startup script (apptainer version)
#
# Usage:
#   ./minimal_reserve.sh                 # First, create a reservation (or copy INSTANCE_URI over)
#   ./minimal_sender_apptainer.sh [OPTIONS]
#
...
echo "Starting E2SAR sender (apptainer)..."
...
echo ""
echo "Running: apptainer run --env EJFAT_URI=<redacted> --env MALLOC_ARENA_MAX=32 $SIF_PATH e2sar_perf --send --withcp --optimize=sendmmsg --sockets=16 --ip=$SENDER_IP --rate=$RATE --length=$LENGTH --num=$NUM --mtu=$MTU --bufsize=134217728"
echo ""
